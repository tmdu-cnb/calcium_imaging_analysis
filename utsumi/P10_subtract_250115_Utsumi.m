...
% F6=horzcat(F_filtered(:,1:2),F0_pre,F_filtered(:,nF_pre+3:nF_pre+5));行1の式を行2に変更
...
for i = 1:numel(stat)
    ...
    if y_range / x_range <= ratio_threshold
        rows_to_delete = [rows_to_delete, i];
    end
end
...
%F_pre2 = F0_pre + min_F2(:, 1:size(F0_pre, 2));


%F6=horzcat(F_filtered(:,1:2), F_pre2,F_filtered(:,nF_pre+3:nF_pre+5));
F6=horzcat(F_filtered(:,1:2), F0_pre,F_filtered(:,nF_pre+3:nF_pre+5));
