...
if ismember('Var1801', data.Properties.VariableNames)
    cells = data(data.Var1801 == 1, :);
else
    error('The column "Var1801" does not exist in the data table.');
end

% Extract activity columns into separate matrix
cells_activity = cells{:, 2:end-3};

% Extract coordinates of cells
xs = cells{:, end};       % X coordinates (assumed to be the last column)
ys = cells{:, end-1};     % Y coordinates (assumed to be the second-to-last column)
...
scatter(ys, xs, 30, cmap(idx, :), 'filled', 'MarkerEdgeColor', 'k');  % Filled circles with black edges

% Label clusters with numbers (white text with black border)
for Ki = 1:num_clusters
    cluster_indices = find(idx == Ki);
    % Plot black text for the border
    text(mean(ys(cluster_indices)), mean(xs(cluster_indices)), num2str(Ki), ...
        'FontSize', 15, 'Color', 'k', 'FontWeight', 'bold', 'Margin', 1, 'BackgroundColor', 'w');
    % Plot white text for the inner part
    text(mean(ys(cluster_indices)), mean(xs(cluster_indices)), num2str(Ki), ...
...
for i = 1:n
    ...
    for j = 1:n
        ...
        dx = bsxfun(@minus, cells.Var1803(sorted_indices(i)), cells.Var1803(sorted_indices(j)));
        dy = bsxfun(@minus, cells.Var1802(sorted_indices(i)), cells.Var1802(sorted_indices(j)));
        dis = sqrt(dx.^2 + dy.^2);
