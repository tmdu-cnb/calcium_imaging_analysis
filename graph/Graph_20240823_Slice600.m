...
if ismember('Var601', data.Properties.VariableNames)
    cells = data(data.Var601 == 1, :);
else
    error('The column "Var601" does not exist in the data table.');
end
...
for i = 1:n
    ...
    for j = 1:n
        ...
        dx = bsxfun(@minus, cells.Var603(sorted_indices(i)), cells.Var603(sorted_indices(j)));
        dy = bsxfun(@minus, cells.Var602(sorted_indices(i)), cells.Var602(sorted_indices(j)));
        dis = sqrt(dx.^2 + dy.^2);
